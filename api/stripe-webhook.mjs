...
// db/schema.ts
var revela = pgSchema("revela");
var sessions = revela.table("sessions", {
...
var events = revela.table("events", {
  id: serial("id").primaryKey(),
...
function drizzle(...params) {
  ...
  if (isConfig(params[0])) {
    ...
    if (typeof connection2 === "object" && connection2.url !== void 0) {
      ...
      const instance22 = src_default(url, config2);
...
function verifyStripeSignature(rawBody, signatureHeader, secret, toleranceSeconds = 300, nowSeconds = Math.floor(Date.now() / 1e3)) {
  if (!rawBody || !signatureHeader || !secret) return false;
  const pieces = signatureHeader.split(",").map((piece) => piece.trim());
  const timestamp2 = Number(pieces.find((piece) => piece.startsWith("t="))?.slice(2));
  const signatures = pieces.filter((piece) => piece.startsWith("v1=")).map((piece) => piece.slice(3)).filter(Boolean);
  if (!Number.isFinite(timestamp2) || signatures.length === 0) return false;
  if (Math.abs(nowSeconds - timestamp2) > toleranceSeconds) return false;
  const expected = createHmac("sha256", secret).update(`${timestamp2}.${rawBody}`, "utf8").digest("hex");
...
var OFFER_AMOUNT_CENTS = 2900;
var OFFER_CURRENCY = "usd";
var TOKEN_PATTERN = /^rv_[a-z0-9]{8,60}$/i;
...
async function handler(request, response) {
  ...
  const eligible = Boolean(checkout?.id) && TOKEN_PATTERN.test(token) && checkout?.mode === "payment" && checkout?.payment_status === "paid" && checkout?.amount_total === OFFER_AMOUNT_CENTS && checkout?.currency?.toLowerCase() === OFFER_CURRENCY;
  ...
  await db.insert(sessions).values({
    ...
    set: {
      email: checkout.customer_details?.email ?? void 0,
      stage: "report",
      completed: true,
      lastSeenAt: now
    }
